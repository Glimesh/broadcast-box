import { ChatAdapter, ChatStatus, Message } from '../../../hooks/useChatSession'
...
export class ChatDataChannelAdapter implements ChatAdapter {
  ...
  attachChannel(channel: RTCDataChannel) {
    ...
    this.onStatus?.(this.channel.readyState === 'open' ? 'connecting' : 'connecting')
  }
  ...
  async send(text: string, displayName: string): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      try {
        ...
        reject(error instanceof Error ? error : new Error('Failed to send chat message'))
      }
    })
  }
}
