...
const Frontpage = () => {
	const [streamType, setStreamType] = useState<'None' | 'Watch' | 'Share'>('None');
	const [streamKey, setStreamKey] = useState('')
	const navigate = useNavigate()

	const onStreamKeyChange = (e: ChangeEvent<HTMLInputElement>) => setStreamKey(e.target.value)
	const onStreamClick = () => {
		if(streamKey === ''){
			return;
		}
			...
			<form className='rounded-md bg-gray-800 shadow-md p-8'>
				...
				{streamType !== "None" && (
					<div className='flex flex-col my-4 justify-center'>
						...
						<button
							className={`py-2 px-4 ${streamKey.length === 0 ? "bg-gray-700" : "bg-blue-500"} text-white font-semibold rounded-lg shadow-md ${streamKey.length === 0 ? "hover:bg-gray-600" : "hover:bg-blue-700" } focus:outline-hidden focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75`}
